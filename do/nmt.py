...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'gru_cond_dad': ('param_init_gru_cond_dad', 'gru_cond_layer_dad'),
          }
...
# Conditional GRU layer with Attention
def param_init_gru_cond_dad(options, params, prefix='gru_cond_dad',
                        nin=None, dim=None, dimctx=None,
                        nin_nonlin=None, dim_nonlin=None):
    if nin is None:
        nin = options['dim']
...
def gru_cond_layer_dad(tparams, state_below, options, prefix='gru',
                   mask=None, context=None, one_step=False,
                   init_memory=None, init_state=None,
                   context_mask=None, ti=0, trng=None, use_noise=None, use_dad=False,
                   **kwargs):
    ...
    def _step_slice(m_, x_, xx_, i, reference_embs,
        ...
        if last_word_index_vec.shape[0] == 0:
            ...
            last_dad_word_embs = last_dad_word_embs.reshape([n_samples, options['dim_word']])
    ...
    current_step_idx = tensor.arange(nsteps) + 1
    seqs = [mask, state_below_, state_belowx, current_step_idx, state_below]
    #seqs = [mask, state_below_, state_belowx, state_belowc]
    _step = _step_slice
    ...
    #
    #     seqs = [mask[i], state_below_[i], state_belowx[i]]
    #     # seqs = [mask, state_below_, state_belowx, state_belowc]
    #
    #     last_emb = state_below[i]
    #
    #     rval = _step_slice(*(seqs + [init_state, None, None] +
    #                    shared_vars))
...
def build_model(tparams, options, number_of_training_instance=0):
    ...
    ti = tensor.dscalar("ti") # ti to indicator the process of training
    ...
    proj = get_layer('gru_cond_dad')[1](tparams, emb, options,
                                            prefix='decoder',
