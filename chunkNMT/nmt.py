...
def prepare_training_data(seqs_x, seqs_y_c, seqs_y_cw, maxlen_chunk=None, maxlen_cw=None, n_words_src=30000,
    ...
    last_word_in_last_chunk = numpy.zeros((maxlen_y_c, n_samples)).astype('int64')
    ...
    for idx, [s_x, s_y_c, s_y_cw] in enumerate(zip(seqs_x, seqs_y_c, seqs_y_cw)):
        ...
        for idx_cw, s_y_cw_i in enumerate(s_y_cw):
            ...
            last_word_in_last_chunk[idx_cw + 1, idx] = s_y_cw_i[-1] # just store the last word of current chunk words, we have shift it here

    # print y_cw

    return x, x_mask, y_c, y_mask_c, y_cw, y_mask_cw, last_word_in_last_chunk
...
def gru_cond_layer(tparams, emb, chunk_index, options, prefix='gru',
                   ...
                   chunk_ctx=None, chunk_alpha=None,
                   **kwargs):
    ...
    def _step_slice(m_, x_, xx_,
                    h_, ctx_, alpha_,
                    pctx_, cc_, chunk_ctx, chunk_alpha,
    ...
    def _chunk_step_slice(chunk_m_, cw_m_, chunk_index, cw_x_, cw_xx_,  # seq
        ...
        rval_chunk_words, updates = theano.scan(_step,
                                    ...
                                    outputs_info=[h_cw[-1],
                                                  ctx_cw[-1],
                                                  alpha_cw[-1]],
                                    non_sequences=[pctx_cw, cc, ctx_, alpha.T]+[W_current_chunk_hidden, W_current_chunk_c, U, Wc, W_comb_att, W_cu_chunk_att, U_att, c_tt, Ux, Wcx, U_nl, Ux_nl, b_nl, bx_nl, h2], # hidden of chunk should help word disambiguation
                                    name=_p(prefix, '_layers'),
    ...
    if one_step_chunk:
        ...
        rval = _step_slice(*(seqs + [init_state_chunk_words, None, None, pctx_, context, chunk_ctx, chunk_alpha] +
...
def build_model(tparams, options):
    ...
    last_word_in_chunk = tensor.matrix('last_word_in_chunk', dtype='int64')
    ...
    last_word_emb = tparams['Wemb'][last_word_in_chunk.flatten()]
    last_word_emb = last_word_emb.reshape([n_timesteps_chunk, n_samples, options['dim_word']])
    ...
    # assert numpy.all( f_with_print([[[1,2]],[[2,4]]]))No such file or directory
    ...
    # prepare scan arguments
    seqs = [emb, last_word_emb]
    init_states = [ tensor.zeros_like(emb[0]) ]
    _step = _step_get_predict_word_embedding
    nsteps = emb.shape[0]
    ...
    return trng, use_noise, x, x_mask, y_chunk, y_chunk_mask, y_chunk_words, \
           y_chunk_words_mask, last_word_in_chunk, opt_ret, cost
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    emb_last_word_in_chunk = tensor.switch(last_word_in_chunk[:, None] < 0,
                        tensor.alloc(0., 1, tparams['Wemb_dec'].shape[1]),
                        tparams['Wemb_dec'][last_word_in_chunk])
    ...
    # weighted averages of context, generated by attention module
    ctxs_chunk =  proj_chunk[1]

    alpha_chunk = proj_chunk[2]
    ...
    proj_word = get_layer(options['decoder'])[1](tparams, emb_chunk_word, None,  options,
                                            ...
                                            chunk_ctx=ctxs_chunk,
                                            chunk_alpha=alpha_chunk)


    emb_chunk_word = tensor.switch(y_chunk_words[:, None] < 0,
                        emb_last_word_in_chunk,
                        emb_chunk_word)
    ...
    logit_prev_chunk = get_layer('ff')[1](tparams, last_chunk_true, options,
                                    prefix='ff_logit_prev_chunk', activ='linear')
    logit_ctx_chunk = get_layer('ff')[1](tparams, ctxs_chunk, options,
                                   prefix='ff_logit_ctx_chunk', activ='linear')
    logit_chunk = tensor.tanh(logit_lstm_chunk+logit_prev_chunk+logit_ctx_chunk)
    if options['use_dropout']:
        logit_chunk = dropout_layer(logit_chunk, use_noise, trng)
    ...
    outs = [probs_chunk, next_sample_chunk, proj_h, ctxs_chunk, alpha_chunk]
    f_next_chunk = theano.function(inps, outs, name='f_next_chunk', profile=profile)
    inps = [last_word_in_chunk, y_chunk_words, ctx, init_state_chunk_words, current_chunk_hidden, ctxs_chunk, alpha_chunk]
...
def gen_sample(tparams, f_init, f_next_chunk, f_next_word, x,
    ...
    last_word_in_chunk = -1 * numpy.ones((1,)).astype('int64')  # bos indicator
    ...
    for ii_chunk in xrange(maxlen_chunks):
        ...
        next_p_chunk, next_chunk, next_state_chunk, chunk_ctx, chunk_alpha= ret[0], ret[1], ret[2], ret[3], ret[4]
        ...
        if stochastic:
            ...
            for ii_chunk in xrange(maxlen_words):
                ...
                inps = [last_word_in_chunk, next_word, ctx, next_state_word, next_state_chunk, chunk_ctx, chunk_alpha]
...
def train(dim_word=100,  # word vector dimensionality
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y_chunk, y_cw in train:
            ...
            x, x_mask, y_c, y_mask_c, y_cw, y_mask_cw, last_word_in_chunk = prepare_training_data(x, y_chunk, y_cw, maxlen_chunk=maxlen_chunk, maxlen_cw=maxlen_chunk_words,
            ...
            cost = f_grad_shared(x, x_mask, y_c, y_mask_c, y_cw, y_mask_cw, last_word_in_chunk)
